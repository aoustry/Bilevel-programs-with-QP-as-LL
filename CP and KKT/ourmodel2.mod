...
#cuts
s.t. cut {k in CUT}: - v + 0.5*sum{i in V}(sum{j in V}(x[i]*x[j]*Q1[i,j]))+ sum{i in V}(q1[i]*x[i]) <= 0.5*sum{i in V}(sum{j in V}(y_inner[k,i]*y_inner[k,j]*Q2_fix[i,j]))+0.5*sum{i in V}(y_inner[k,i]*y_inner[k,i]*diagQ2[i]*x[i])+sum{i in V}(q2_fix[i]*y_inner[k,i])+sum{i in V}(sum{j in V}(x[i]*y_inner[k,j]*M[j,i]));
