...
param M {i in V, j in V}; #adjacency matrix
...
s.t. matrix_const {i in V, j in V}: Q2[i,j]=Q2_fix[i,j]; #: i!=j
#s.t. vector {i in V: i=2,5}: q2[i]=x[i];
s.t. vector_const {i in V}: q2[i]=q2_fix[i]; #: i!=2,5
#cuts
...
#constraints
s.t. s.t. simplex_y: sum{i in V} y[i]=1;
