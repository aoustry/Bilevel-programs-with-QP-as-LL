...
#cuts
s.t. cut {k in CUT}: sum{i in N}(0.5*sum{j in N}(y_inner[k,i]*y_inner[k,j]*Q[i,j])) + sum{i in N}(q[i]*y_inner[k,i])+c >= 0;
...
#obj function
minimize obj_inner: sum{i in N}(0.5*sum{j in N}(y[i]*y[j]*Q_star[i,j])) + sum{i in N}(q_star[i]*y[i])+c_star;
