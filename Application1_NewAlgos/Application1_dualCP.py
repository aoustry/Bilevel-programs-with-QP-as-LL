# -*- coding: utf-8 -*-
"""
Created on Mon Dec  6 14:12:27 2021
...
def save(name, p,value,value_tested, soltime, bigQ, q,c):
    f = open("Application1_data/"+name+"/dualCP_obj_value.txt","w+")
...
def main(name):
    ...
    Qxk_list, qxk_list, vxk_list,running = [],[],[],True
    k=1
    while running:
        
        Qsol,qsol,csol = master(name,n,wlist,z,Qxk_list,qxk_list,np.array(vxk_list))
        y,val = solve_subproblem_App1(n,Qsol,qsol,csol)
        Qxk_list.append(Qsol)
        qxk_list.append(qsol)
        vxk_list.append(val-csol)
        # y2,val2 = solve_subproblem_App1(n,Qsol-mu*np.eye(n),qsol,csol)
        # Qxk_list.append(Qsol-mu*np.eye(n))
        # qxk_list.append(qsol)
        # vxk_list.append(val2-csol)
        # if abs(val)<1E-6:
        #     running=False


def master(name,n,wlist,z,Qxk_list,qxk_list, vxk_vector):
    p = len(z)
