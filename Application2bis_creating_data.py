...
def create_files(name,n,Q1,Q2,q1,q2,M,diagonalQ2x):
   #Reading graph file
    ...
    #Write the .dat file
    os.mkdir("Application2bis_data/"+name)
    f = open("Application2bis_data/"+name+"/instance.dat","w")
    f.write("param n := %d;\n"%n)
    f.write("\nparam M :")
    for i in range(n):
        f.write(" %d "%(i+1))
    ...
    np.save("Application2bis_data/"+name+"/bigQ1",Q1)
    np.save("Application2bis_data/"+name+"/bigQ2_fix",Q2)
    np.save("Application2bis_data/"+name+"/q1",q1)
    np.save("Application2bis_data/"+name+"/q2_fix",q2)
    np.save("Application2bis_data/"+name+"/M",M)
    np.save("Application2bis_data/"+name+"/diagQ2x",diagonalQ2x)
